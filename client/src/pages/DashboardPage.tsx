...
export function DashboardPage() {
  ...
  const { items, isFetching, isAdding, error } = useAppSelector(
    (s) => s.repositories,
  );
  ...
  const handleAdd = (values: AddFormValues) => {
    ...
    void dispatch(addRepository({ owner, name })).then((action) => {
      if (addRepository.fulfilled.match(action)) {
        ...
        void message.error(
          (action.payload as string) || t("errors.repositories.addFailed"),
        );
      }
    });
  };
      ...
      ...langs.map((lang) => ({
        ...
        label:
          lang === UNKNOWN_LANGUAGE
            ? t("dashboard.filters.unknownLanguage")
            : lang,
      })),
    ];
  }, [items, t]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((repo) => {
      ...
      const matchesLanguage =
        languageFilter === "all" || repoLanguage === languageFilter;
        ...
        render: (url: string) => (
          <div className="url-cell">
            ...
            <Tooltip title={t("dashboard.table.openInNewTab")}>
              ...
              >
                <span className="url-go-icon" aria-hidden="true">
                  ↗
                </span>
                  ...
                  void dispatch(refreshRepository(record.id)).then((action) => {
                    if (refreshRepository.fulfilled.match(action)) {
                      void message.success(
                        t("errors.repositories.refreshSuccess"),
                      );
                    } else {
                      void message.error(
                        t("errors.repositories.refreshFailed"),
                      );
                    }
                  })
                }
                ...
                onClick={() =>
                  void dispatch(removeRepository(record.id)).then((action) => {
                    if (removeRepository.fulfilled.match(action)) {
                      void message.success(
                        t("errors.repositories.removeSuccess"),
                      );
                    } else {
